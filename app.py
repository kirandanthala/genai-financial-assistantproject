...
def ask_ui():
    ...
    if "amount" not in df.columns or "category" not in df.columns:
        return render_template("index.html", answer="Error: transactions.csv missing required columns")
    ...
    for cat in df["category"].dropna().unique():
        if cat.lower() in user_text:
            ...
            if "date" in df.columns and (month_num or year):
                if month_num:
            ...
            if filtered.empty:
                context = f"No spending records found for category '{cat}'"
            else:
                total = filtered["amount"].sum()
                context = f"You spent {total} INR on {cat}."
            matched = True
            break

    if not matched:
        context = "No records found for the category mentioned in your query."
    ...
    try:
        completion = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "You are a financial assistant. Answer based only on transaction context."},
